...
struct SqliteEventRepository::Impl {
  explicit Impl(const std::size_t max_pending) : max_pending_appends(max_pending) {}
...
Error overload_error() {
  return Error{
      .code = std::make_error_code(std::errc::resource_unavailable_try_again).value(),
...
template <typename Impl> bool acquire_append_slot(Impl& impl) noexcept {
  ...
  while (pending < impl.max_pending_appends) {
    if (impl.pending_appends.compare_exchange_weak(pending, pending + 1, std::memory_order_relaxed))
      return true;
  }
  return false;
}
...
auto on_database(Impl& impl, Function&& function) -> std::invoke_result_t<Function> {
  using Result = std::invoke_result_t<Function>;
  std::promise<Result> promise;
  auto future = promise.get_future();
  const auto operation = "persistence-sync-" + std::to_string(impl.next_operation.fetch_add(1));
...
SqliteEventRepository::SqliteEventRepository(std::unique_ptr<Impl> impl) : impl_(std::move(impl)) {}

std::expected<std::unique_ptr<SqliteEventRepository>, Error>
SqliteEventRepository::open(const std::filesystem::path& path, const RepositoryOptions options) {
  auto repository = std::unique_ptr<SqliteEventRepository>(
      new SqliteEventRepository(std::make_unique<Impl>(options.max_pending_appends)));
  auto initialized = on_database(
      *repository->impl_,
      [impl = repository->impl_.get(), filename = path.string()]() -> std::expected<void, Error> {
        try {
          impl->connection.connect(filename);
          sqlite3_int64 schema_version = 0;
          for (const auto& [version] :
               sqlite::query<std::tuple<sqlite3_int64>>(impl->connection, "PRAGMA user_version")) {
            schema_version = version;
          }
          if (schema_version > current_schema_version) {
            throw std::runtime_error("database schema version " + std::to_string(schema_version) +
                                     " is newer than supported version " +
                                     std::to_string(current_schema_version));
          }

          impl->connection.execute("PRAGMA journal_mode=WAL;"
                                   "PRAGMA foreign_keys=ON;"
                                   "PRAGMA busy_timeout=5000;");
          if (schema_version < current_schema_version) {
            sqlite::transaction migration{impl->connection};
            impl->connection.execute("CREATE TABLE IF NOT EXISTS symphony_events("
          ...
          impl->connection.execute("SELECT sequence, schema_version, type, issue_id, payload "
...
SqliteEventRepository::~SqliteEventRepository() {
  if (!impl_ || !impl_->connection.valid()) return;
...
std::expected<std::int64_t, Error> SqliteEventRepository::append(NewEvent event) {
  return on_database(
      *impl_,
      [impl = impl_.get(), event = std::move(event)]() -> std::expected<std::int64_t, Error> {
...
SqliteEventRepository::load_after(const std::int64_t sequence) {
  return on_database(
      *impl_, [impl = impl_.get(), sequence]() -> std::expected<std::vector<Event>, Error> {
        try {
          std::vector<Event> events;
          using Row =
              std::tuple<sqlite3_int64, sqlite3_int64, std::string, std::string, std::string>;
          for (auto&& [row_sequence, schema_version, type, issue_id, payload] :
               sqlite::query<Row>(impl->connection,
...
std::expected<void, Error> SqliteEventRepository::submit_append(std::string key, NewEvent event) {
  if (key.empty()) throw std::invalid_argument("persistence operation key must not be empty");
...
std::vector<AppendCompletion> SqliteEventRepository::take_ready_appends() {
  ...
  return std::exchange(impl_->append_completions, std::vector<AppendCompletion>{});
}

void SqliteEventRepository::drain() {
  impl_->tasks.drain();
}
