...
DaemonParseResult parse_daemon_arguments(const int argc, char** argv) {
  CLI::App app{"Standalone C++26 implementation of OpenAI Symphony Draft v1", "symphonyd"};
  std::string positional_workflow;
  std::string named_workflow;
  bool once = false;

  auto* positional =
      app.add_option("workflow_path", positional_workflow,
                     "Path to WORKFLOW.md; defaults to WORKFLOW.md in the current directory");
  auto* named = app.add_option("--workflow", named_workflow,
  ...
  try {
    ...
    return std::unexpected(ParseExit{code, standard_output.str(), standard_error.str()});
  }
