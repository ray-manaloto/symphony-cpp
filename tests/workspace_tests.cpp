...
static ut::suite workspace_tests = [] {
  ut::test("workspace leaf is sanitized and collision resistant") = [] {
    ...
    ut::expect(
        std::ranges::all_of(first.substr(first.size() - 32), [](const unsigned char character) {
          return std::isxdigit(character) != 0;
        }));
  };

  ut::test("workspace leaf preserves allowed identifiers without a suffix") = [] {
    const symphony::domain::Issue issue{"opaque-id", "SYM.1_release-2", "", "Todo", {}};
    ut::expect(symphony::workspace::workspace_leaf(issue) == std::string{"SYM.1_release-2"});
  };

  ut::test("workspace leaf hashes the original identifier with at least 64 bits") = [] {
    const symphony::domain::Issue space{"same-id", "SYM 1", "", "Todo", {}};
    const symphony::domain::Issue slash{"same-id", "SYM/1", "", "Todo", {}};
    const symphony::domain::Issue utf8{"same-id", std::string{"SYM-\xC3\xA9"}, "", "Todo", {}};
    ...
    ut::expect(first != second);
    ut::expect(first == std::string{"SYM_1-5fd1b797e690bf120102dc7e77c6a3ff"});
  ...
  ut::test("workspace leaf rejects a missing required identifier") = [] {
    ...
    ut::expect(ut::throws([&] { static_cast<void>(symphony::workspace::workspace_leaf(issue)); }));
  };

  ut::test("workspace leaf does not emit reserved path components") = [] {
    const symphony::domain::Issue dot{"dot-id", ".", "", "Todo", {}};
    const symphony::domain::Issue dot_dot{"dot-dot-id", "..", "", "Todo", {}};
  ...
  ut::test("fixture workspace rejects a symlink swapped in before hooks or removal") = [] {
    ...
    ut::expect(ut::throws([&] {
      executor.run_hook(workspace, "before_run", {"fixture", "check"}, std::chrono::seconds{1});
    }));
  ...
  ut::test(
      "local workspace rejects traversal and symlink swaps without touching outside data") = [] {
    const auto temporary = std::filesystem::temp_directory_path();
    ...
    const auto workspace = executor.create(issue);
    const symphony::workspace::Workspace forged{root / ".." / outside.filename(), issue.id, false};
    ut::expect(ut::throws([&] {
    ...
    std::filesystem::create_directory_symlink(outside, workspace.path);
    ut::expect(ut::throws([&] {
      executor.run_hook(workspace, "before_run", {"/usr/bin/touch", "escaped"},
                        std::chrono::seconds{1});
    }));
    ut::expect(ut::throws([&] { executor.remove(workspace); }));
    ut::expect(std::filesystem::exists(sentinel));
