...
template <>
struct glz::meta<ProbeHooks> {
  using T = ProbeHooks;
  static constexpr auto value = object("before_run", &T::before_run);
};

template <>
struct glz::meta<ProbeTracker> {
  using T = ProbeTracker;
  static constexpr auto value =
      object("kind", &T::kind, "provider", &T::provider,
             "active_states", &T::active_states);
};

template <>
struct glz::meta<ProbeWorkflow> {
  using T = ProbeWorkflow;
  static constexpr auto value =
      object("hooks", &T::hooks, "tracker", &T::tracker);
};
...
static ut::suite glaze_yaml_tests = [] {
  ut::test("Glaze YAML preserves workflow shapes and provider extensions") = [] {
    ...
    ut::expect(workflow.hooks->before_run ==
               std::optional<std::string>{"echo fixture\n"});
    ...
    const auto write_error =
        glz::write_yaml(workflow.tracker->provider, provider_yaml);
