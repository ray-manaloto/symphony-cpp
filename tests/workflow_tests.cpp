...
static ut::suite workflow_tests = [] {
  ...
  ut::test("workflow ignores extension keys and rejects absent path environment") = [] {
    FakeEnvironment env;
  ...
  ut::test("workflow preserves provider config and normalizes state "
    ...
    ut::expect(document.config.tracker.provider_yaml.find("symphony") != std::string::npos);
  ...
  ut::test("dispatch validation rejects missing and unsupported trackers") = [] {
    ...
    ut::expect(ut::throws([&] { symphony::workflow::validate_for_dispatch(config, {"fake"}); }));
  ...
  ut::test("dispatch validation rejects blank Codex settings") = [] {
    ...
    expect_rejected([](auto& codex) { codex.escalation_reasoning_effort = " "; });
    expect_rejected([](auto& codex) { codex.repeated_failure_reasoning_effort = " "; });
  };
  ...
  ut::test("workflow rejects ineffective context rollover percentages") = [] {
    ...
    for (const auto percent : {0, 100}) {
      const auto path =
          temp_workflow("---\ncodex:\n  context_rollover_percent: " + std::to_string(percent) +
                        "\n---\nprompt\n");
      ut::expect(ut::throws(
          [&] { static_cast<void>(symphony::workflow::WorkflowLoader{}.load(path, env)); }));
      std::filesystem::remove(path);
  ...
  ut::test("strict prompt renderer permits only issue and attempt variables") = [] {
    ...
    ut::expect(ut::throws([&] {
      static_cast<void>(symphony::workflow::render_prompt("{{ secret }}", issue, attempt));
    }));
  };
};
