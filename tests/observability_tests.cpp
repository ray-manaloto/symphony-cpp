...
template <> struct glz::meta<EventProbe> {
  using T = EventProbe;
...
static ut::suite observability_tests = [] {
  ...
  ut::test("structured event store isolates sink failures") = [] {
    ...
    auto recording = std::make_shared<spdlog::sinks::ostream_sink_mt>(output, true);
    auto logger = std::make_shared<spdlog::logger>(
        "failing-fixture", spdlog::sinks_init_list{std::make_shared<FailingSink>(), recording});
    ...
    ut::expect(output.str().find("\"issue_identifier\":\"SYM-1\"") != std::string::npos);
