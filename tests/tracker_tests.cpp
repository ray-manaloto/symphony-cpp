...
static ut::suite tracker_tests = [] {
  ut::test("tracker normalization maps complete issue fields and canonical labels") = [] {
    ...
    ut::expect(normalized->description ==
               std::optional<std::string>{"Fixture only"});
    ut::expect(normalized->labels ==
               std::vector<std::string>({"backend", "ready"}));
  ...
  ut::test("paged tracker collection follows cursors once in stable page order") = [] {
    ...
    ut::expect(cursors == std::vector<std::optional<std::string>>(
                              {std::nullopt, std::string{"next"}}));
  };
  ...
  ut::test("adapter profiles and HTTP failures map to portable contracts") = [] {
    ...
    ut::expect(linear->active_states ==
               std::vector<std::string>({"Todo", "In Progress"}));
    ut::expect(linear->terminal_states ==
               std::vector<std::string>({"Done", "Cancelled"}));
    ...
    ut::expect(unavailable.code ==
               symphony::tracker::TrackerErrorCode::unavailable);
