...
| Dynamic sanitizer analysis | GCC 16.1 ASan/UBSan and separate TSan presets | Implemented with fatal sanitizer findings and frame pointers. Source CI run `29985848551` passed all three registered targets under debug, ASan/UBSan, and TSan; GNU's documented `atomic_thread_fence` instrumentation limitation remains visible as nonfatal `-Wtsan` output only in the TSan build |
