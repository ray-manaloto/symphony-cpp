...
| YAML front matter and prompt split | Glaze YAML shape/provider fixtures, local GCC 16.1 compile-time raw-DTO contracts, workflow tests, and named repository `WORKFLOW.md` CTest | Implemented with typed known fields, extension-key tolerance, generic provider-subtree preservation, block hooks, environment/path policy, and strict prompt separation; exact Source CI run `29987362658` passed the prior YAML debug, ASan/UBSan, and TSan gates. The repository workflow carries both normative `codex.stall_timeout_ms` and the comparative OpenSymphony `agent.stall_timeout_ms` extension; record exact published DTO-contract evidence after this slice's Source CI passes. |
