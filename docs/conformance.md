...
| YAML front matter and prompt split | Glaze YAML shape/provider fixtures, local GCC 16.1 compile-time raw-DTO contracts, workflow tests, and named repository `WORKFLOW.md` CTest | Implemented with optional front matter, one trimmed nonempty prompt body, typed known fields, extension-key tolerance, generic provider-subtree preservation, block hooks, and environment/path policy. Delimiter-led malformed front matter remains fail-closed rather than being reinterpreted as a prompt. The repository workflow carries both normative `codex.stall_timeout_ms` and the comparative OpenSymphony `agent.stall_timeout_ms` extension; exact Source CI run `30148754096` passed the §5.2 correction under GCC 16.1 Debug, Release, ASan/UBSan, and TSan. |
...
| Codex context telemetry | protocol, conversation, supervisor-policy, workflow, and scheduler fixtures | Implemented for distinct optional last-turn input decoding, complete cumulative-shape and signed provider-count validation, thread/turn correlation, schema-valid stale-turn rejection, malformed-frame failure, and between-turn freshness reset, cumulative cost-accounting preservation, conservative standalone cumulative rollover, fail-closed pure-supervisor last-turn pressure, model context-window preservation, legacy/current compaction-event counting, strict threshold validation, operator events, and fresh-session rollover after observed compaction; external-supervisor process wiring remains open |
...
| Dynamic sanitizer analysis | GCC 16.1 ASan/UBSan and separate TSan presets; fixture-only Clang 22.1.8 RTSan provider gate | GCC authority is implemented with fatal sanitizer findings and frame pointers. Source CI run `29985848551` passed all registered targets under debug, ASan/UBSan, and TSan; GNU's documented `atomic_thread_fence` instrumentation limitation remains visible as nonfatal `-Wtsan` output only in the TSan build. The Clang Function Effect/RTSan safe and negative controls remain pending exact Linux-image evidence and do not claim any product path is real-time-safe. |
