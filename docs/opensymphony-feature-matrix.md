...
| Live scheduler canary | Adopt exactly once after reset admission | Historical pre-reset `GUI-5` is consumed. The owner separately authorizes exactly one post-reset fixture-only canary after every admission row passes; no second post-reset issue may be activated |
